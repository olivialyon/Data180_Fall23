...
```{r}
barplot(table(Cars93$Type), ylab="Count", xlab = "Car Type", cex.lab = 1.2, cex.names = 1.2, main = "Frequency of Car Type", col=c(2,3,4,5,6,7))
```
...
```{r}
pie(table(Cars93$Type),main = "Frequency of Car Type", col=c(2,3,4,5,6,7), cex.lab=1.2)
```
...
```{r}
hist(Cars93$Horsepower, ylab="frequency",xlab = "Horsepower", cex.axis=1.2, cex.lab =1.2, main = "Horsepower in Cars", col="red")
```
...
```{r}
hist(Cars93$Horsepower, ylab="frequency",xlab = "Horsepower", cex.axis=1.2, cex.lab =1.2, main = "Horsepower in Cars", col="red",breaks=seq(50,300,25) )
typeof(Cars93$'Horsepower')
```
...
```{r}
sqrt(93)
# The second histogram because the amount of bars =10, and the rule of thumb for the number of bars is sqrt(n), in this case it is sqrt(93), which =9.6.
```
...
# this data should be made into a bar plot, not a histogram because it is discrete. 
```
...
```{r}
barplot(table(Cars93$Passengers), xlab = 'Number of Passengers', ylab= 'Frequency', main = 'Number of Passengers in Cars93', col="pink")
...
```{r}
hist(Cars93$Length, xlab = "Length", cex.lab=1.2, col="cyan", main = "Length of Cars93")
```
...
# yes since the mean and the median are very close in value, which is expected from a symmetrically distributed set of data
```
